...
func _process(delta):
	#print("Player pos: "+str(player.global_position)+ "Cam pos: "+str(global_position))
	while(player.global_position.x<global_position.x-Global_Constants.SCREEN_SIZE.x/2):
		position.x-=Global_Constants.SCREEN_SIZE.x
	while(player.global_position.x>global_position.x+Global_Constants.SCREEN_SIZE.x/2):
		position.x+=Global_Constants.SCREEN_SIZE.x
	while(player.global_position.y<global_position.y-Global_Constants.SCREEN_SIZE.y/2):
